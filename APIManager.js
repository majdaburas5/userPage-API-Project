class APIManager {
  ...
  loadData() {
    return $.get(`https://randomuser.me/api/`).then((randomUser) => {
      ...
      return $.get(`https://api.kanye.rest/`).then((quoteRes) => {
        ...
        return $.get(`https://randomuser.me/api/?results=6`).then(
          (friendUsers) => {
            const friends = [];
            for (let i = 0; i < friendUsers.results.length; i++) {
              const firstName = friendUsers.results[i].name.first;
              const lastName = friendUsers.results[i].name.last;
